...
# 2. Google Gemini & Telegram Setup
...
def start_trading(message):
    ...
    bot.reply_to(message, "Set your balance before requesting a signal.\n\n💰 Enter your current balance. Example: 1000")
...
def handle_duration(message):
    if message.chat.id not in user_data or 'balance' not in user_data[message.chat.id]:
        user_data[message.chat.id] = {'state': 'WAITING_BALANCE'}
        bot.reply_to(message, "⚠️ Set balance first.\n💰 Enter balance (e.g. 1000):")
        return
...
def process_image_fast(message, file_path, chat_id):
    try:
        # इन-मेमोरी फ़ास्ट डाउनलोड (डिस्क सेव नहीं होगा)
        ...
        prompt = (
            f"You are MAGNATE AI. Expiry: {duration}. "
            f"Fast technical analysis: Check candle momentum, S/R level, and indicators. "
            f"If unclear/choppy, output SKIP. "
            f"Format strictly:\n"
            f"• Recommendation: UP / DOWN / SKIP\n"
            f"• Reason: 2 crisp sentences explaining setup and levels."
        )
        
        # Google Gemini का सबसे तेज़ मॉडल
...
def analyze_chart(message):
    ...
    if chat_id not in user_data or 'balance' not in user_data[chat_id]:
        bot.reply_to(message, "⚠️ Set your balance first.\nClick 🚀 Start trading below.", reply_markup=main_menu())
        return

    bot.reply_to(message, "⏳ Analyzing chart screenshot, please wait...")
    file_info = bot.get_file(message.photo[-1].file_id)
    
    t = threading.Thread(target=process_image_fast, args=(message, file_info.file_path, chat_id))
    t.start()

if __name__ == "__main__":
    print("Magnate AI running ultra fast on Gemini...")
