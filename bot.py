...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    selected_name = [k for k, v in ASSETS_MAP.items() if v == selected_code] or [selected_code]
    ...
    await query.edit_message_text(
        text=f"✅ **सफलतापूर्वक सेट हुआ!**\n\n📡 बोट अब सिर्फ **{current_target_name}** (`{current_target_pair}`) को 24/7 स्कैन कर रहा है।\nजैसे ही स्ट्रेटेजी कन्फर्म होगी, तुरंत अलर्ट आएगा।",
        parse_mode="Markdown"
    )
...
def analyze_market(df, pair_name):
    ...
    if up_signal and last_alert_signal != "UP":
        ...
        send_telegram_alert(current_target_name, "STRONG DOWN (PUT 🔴)", current_price)
                    ...
                    if isinstance(data, dict) and "d" in data:
                        for item in data["d"]:
                            if isinstance(item, dict) and "p" in item and "q" in item:
                                ...
                                if current_target_pair in pair or pair in current_target_pair or pair == current_target_pair:
                                    price = float(item["q"])
                                    t_stamp = item.get("t", 0)
                                    new_row = {"time": t_stamp, "open": price, "high": price, "low": price, "close": price}
